...
require('@antv/matrix-util');

function _interopDefaultLegacy (e) { return e && typeof e === 'object' && 'default' in e ? e : { 'default': e }; }

var ___default = /*#__PURE__*/_interopDefaultLegacy(_$e);
var d3Force__default = /*#__PURE__*/_interopDefaultLegacy(d3Force);
  ...
  _createClass(BaseEndpoint, [{
    ...
    value: function updatePos() {
      ...
      if (this._isInitedDom) {
        // 计算width,height,left,top
        this._width = $(this.dom).outerWidth();
        this._height = $(this.dom).outerHeight(); // 计算锚点起始值
...
function calcSlope(opts) {
  var shapeType = ___default['default'].get(opts, 'shapeType');

  var dom = ___default['default'].get(opts, 'dom');

  var arrowPosition = ___default['default'].get(opts, 'arrowPosition', 0.5);

  var path = ___default['default'].get(opts, 'path');
...
var BaseEdge = /*#__PURE__*/function (_Edge) {
  ...
  function BaseEdge(opts) {
    ...
    _this._UPDATE_INTERVAL = 20; // 线段起始位置

    _this._sourcePoint = null;
    _this._targetPoint = null; // 线段的z-index
  ...
  _createClass(BaseEdge, [{
    ...
    key: "on",
    value: function on(type, callback) {
      _get(_getPrototypeOf(BaseEdge.prototype), "on", this).call(this, type, callback);
    ...
    value: function _addEventListener() {
      ...
      var _emitEvent = function _emitEvent(e) {
        e.preventDefault();
        e.stopPropagation();
      ...
      if (this.isExpandWidth) {
        $$3(this.eventHandlerDom).on('click', _emitEvent);
      } else {
        $$3(this.dom).on('click', _emitEvent);
      }
    }
  }, {
    key: "_create",
...
var BaseGroup = /*#__PURE__*/function (_Group) {
  ...
  function BaseGroup(opts) {
    ...
    _this.endpoints = [];
    _this._endpointsData = opts.endpoints; // 记录节点是否在拖动
  ...
  _createClass(BaseGroup, [{
    ...
    key: "on",
    value: function on(type, callback) {
      _get(_getPrototypeOf(BaseGroup.prototype), "on", this).call(this, type, callback);
...
function forceLayout(param) {
  ...
  var simulation = d3Force__default['default'].forceSimulation(nodes).force('charge', function () {
    if (opts.chargeStrength) {
      return d3Force__default['default'].forceManyBody().strength(opts.chargeStrength);
    } else {
      return d3Force__default['default'].forceManyBody();
    }
  }()).force('center', d3Force__default['default'].forceCenter(opts.width / 2, opts.height / 2)).force('link', d3Force__default['default'].forceLink(edges).id(function (node) {
...
function concentLayout(param) {
  var center = param.center,
      ...
      param.sweep;
      var equidistant = param.equidistant,
...
function drageLayout(param) {
  ...
  if (!nodes) return;
  var g = new dagre__default['default'].graphlib.Graph();
  var nodeSizeFunc;
...
var MDS = /*#__PURE__*/function () {
  ...
  _createClass(MDS, [{
    ...
    value: function layout() {
      ...
      try {
        // square distances
        var M = mlMatrix.Matrix.mul(mlMatrix.Matrix.pow(distances, 2), -0.5); // double centre the rows/columns
        ...
        var ret = new mlMatrix.SingularValueDecomposition(M);
        var eigenValues = mlMatrix.Matrix.sqrt(ret.diagonalMatrix).diagonal();
        return ret.leftSingularVectors.toJSON().map(function (row) {
          return mlMatrix.Matrix.mul([row], [eigenValues]).toJSON()[0].splice(0, dimension);
        });
      } catch (_unused) {
        var res = [];
  ...
  _createClass(RadialNonoverlapForce, [{
    ...
    value: function updatePositions() {
      ...
      positions.forEach(function (n, i) {
        ...
        if (distLength > 0 && i !== f) {
          var limitedDist = Math.min(maxDisplace * (speed / SPEED_DIVISOR$1), distLength);
...
function RadialLayout(params) {
  var self = params.opts;
  var nodes = params.data.nodes;
  var edges = params.data.edges || [];
  var center = self.center;
  ...
  var maxDistance = maxToFocus(D, focusIndex); // replace first node in unconnected component to the circle at (maxDistance + 1)
  ...
  self.radii = radii;
  var eIdealD = eIdealDisMatrix(params); // const eIdealD = scaleMatrix(D, linkDistance);
...
function oneIteration(param, positions, radii, D, W, params) {
  var self = params.opts;
  var vparam = 1 - param;
...
var CoordinateService = /*#__PURE__*/function () {
  function CoordinateService(opts) {
    ...
    this.scale = opts.scale || 1; // 滚动高度和左右

    this.scrollTop = 0;
    this.scrollLeft = 0;
    this.scrollTimer = undefined;

    this._calcScrollPos(); // 中心点
...
var GridService = /*#__PURE__*/function () {
  ...
  _createClass(GridService, [{
    ...
    value: function createCircle() {
      var _ctx = this.dom.getContext('2d'); // _ctx.fillStyle = "rgba(0, 0, 0, 1)";
...
var DOT_COLOR = 'rgba(246, 105, 2, 1)';
...
var BaseCanvas = /*#__PURE__*/function (_Canvas) {
  ...
  function BaseCanvas(options) {
    ...
    _this.theme = {
      group: {
        type: _$c.get(options, 'theme.group.type') || 'normal'
      },
      edge: {
      ...
      endpoint: {
        ...
        expandArea: {
          left: _$c.get(options, 'theme.endpoint.expandArea.left') === undefined ? 10 : _$c.get(options, 'theme.endpoint.expandArea.left'),
          right: _$c.get(options, 'theme.endpoint.expandArea.right') === undefined ? 10 : _$c.get(options, 'theme.endpoint.expandArea.right'),
          top: _$c.get(options, 'theme.endpoint.expandArea.top') === undefined ? 10 : _$c.get(options, 'theme.endpoint.expandArea.top'),
          bottom: _$c.get(options, 'theme.endpoint.expandArea.bottom') === undefined ? 10 : _$c.get(options, 'theme.endpoint.expandArea.bottom')
        }
      },
    ...
    _this._rootHeight = $$9(_this.root).height();
    $$9(_this.root).css('overflow', 'hidden');

    if ($$9(_this.root).css('position') === 'static') {
      $$9(_this.root).css('position', 'relative');
    } // 节点,线段,节点组z-index值，顺序：节点 > 线段 > 节点组


    _this._dragGroupZIndex = 50;
    _this._dragNodeZIndex = 250;
    _this._dragEdgeZindex = 499;
    _this._isInitEdgeZIndex = false; // 检测节点拖动节点组的hover状态

    _this._hoverGroupQueue = [];
    _this._hoverGroupObj = undefined;
    _this._hoverGroupTimer = undefined; // 网格布局
    ...
    _this._bgObjQueue = [];
    _this._bgObj = undefined;
    _this._bgTimer = undefined; // 坐标转换服务
  ...
  _createClass(BaseCanvas, [{
    ...
    value: function addEdges(links, isNotEventEmit) {
      ...
      var result = links.map(function (link) {
        ...
        if (link.type === 'endpoint') {
          ...
          var _sourceNode2 = _this5.getNode(link.source);

          var _targetNode = _this5.getNode(link.target);

          if (!_sourceNode2 || !_targetNode) {
            console.warn("butterflies error: can not connect edge. link sourceId:".concat(link.source, ";link targetId:").concat(link.target));
            return;
          }
    ...
    value: function getNeighborEdgesByEndpoint(id, pointId) {
      var edges = this.getNeighborEdges(id);
      return edges.filter(function (item) {
        if (item.type === 'node') {
          return false;
        } else {
          return item.sourceNode.id === id && item.sourceEndpoint.id === pointId || item.targetNode.id === id && item.targetEndpoint.id === pointId;
        }
      });
    }
  }, {
    ...
    value: function getNeighborNodesAndEdgesByLevel(_ref) {
      ...
      var _loop = function _loop() {
        ...
        (Object.values(_$c.get(adjTable, $node.id, {})) || []).forEach(function (neighborsWithEndpoint) {
    ...
    value: function focusNodesWithAnimate(param) {
      ...
      if (_$c.get(options, 'keepPreZoom')) {
    ...
    value: function setGirdMode() {
      ...
      var flat = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this._bgObj;

      var _isResize = arguments.length > 2 ? arguments[2] : undefined;
      ...
      if (flat) {
        ...
        this._bgTimer = setInterval(function () {
          if (_this15._bgObjQueue.length === 0) {
            clearInterval(_this15._bgTimer);
            _this15._bgTimer = null;
            return;
          }

          _this15._bgObj = _this15._bgObjQueue.pop();
          _isResize && _this15._gridService._resize();

          _this15._gridService.create(_this15._bgObj);
    ...
    value: function setGuideLine() {
      ...
      if (flat) {
        ...
        this._bgTimer = setInterval(function () {
          if (_this16._bgObjQueue.length === 0) {
            clearInterval(_this16._bgTimer);
            _this16._bgTimer = null;
            return;
          }

          _this16._bgObj = _this16._bgObjQueue.pop();

          _this16._guidelineService.create(_this16._bgObj);
    ...
    value: function setMinimap() {
      ...
      var getNodes = function getNodes() {
        return _this17.nodes.map(function (node) {
          return {
            id: node.id,
            left: node.left,
            top: node.top,
      ...
      var getGroups = function getGroups() {
        return _this17.groups.map(function (group) {
          return {
            id: group.id,
            left: group.left,
            top: group.top,
      ...
      if (flat && !this.minimap) {
        ...
        this.updateFn = function () {
          _this17.minimap.update({
            nodes: getNodes(),
            groups: getGroups(),
            zoom: _this17.getZoom(),
            offset: _this17.getOffset()
          });
        };
    ...
    value: function add2Union(name, obj) {
      ...
      if (obj.nodes) {
        obj.nodes.forEach(function (item) {
          ...
          var node = isId ? _this18.getNode(item) : item;
      ...
      if (obj.groups) {
        obj.groups.forEach(function (item) {
          ...
          var group = isId ? _this18.getGroup(item) : item;
      ...
      if (obj.edges) {
        obj.edges.forEach(function (item) {
          ...
          var edge = isId ? _this18.getEdge(item) : item;
    ...
    value: function _addEventListener() {
      ...
      var _isChrome = /Chrome/.test(window.navigator.userAgent) && /Google Inc/.test(window.navigator.vendor);
      ...
      if (_isHightVerChrome && window.ResizeObserver && this.theme.autoResizeRootSize) {
        ...
        var _resizeObserver = new ResizeObserver(function (entries) {
          _this19._rootWidth = $$9(_this19.root).width();
          _this19._rootHeight = $$9(_this19.root).height();

          _this19._coordinateService._changeCanvasInfo({
            terOffsetX: $$9(_this19.root).offset().left,
            terOffsetY: $$9(_this19.root).offset().top,
            terWidth: $$9(_this19.root).width(),
            terHeight: $$9(_this19.root).height()
          });

          _this19.canvasWrapper.resize({
            root: _this19.root
          });

          _this19.setGirdMode(true, undefined, true);
        });
      ...
      this.on('InnerEvents', function (data) {
        if (data.type === 'node:addEndpoint') {
          _this19._addEndpoint(data.data, 'node', data.isInited);
        } else if (data.type === 'node:removeEndpoint') {
          var _point = data.data;

          var rmEdges = _this19.edges.filter(function (item) {
          ...
          _this19.removeEdges(rmEdges);
        } else if (data.type === 'group:addEndpoint') {
          _this19._addEndpoint(data.data, 'group', data.isInited);
        } else if (data.type === 'node:dragBegin') {
          _this19._dragType = 'node:drag';
          _this19._dragNode = data.data;
        } else if (data.type === 'node:mouseDown') {
          _this19._dragType = 'node:mouseDown';
        } else if (data.type === 'group:dragBegin') {
          _this19._dragType = 'group:drag';
          _this19._dragNode = data.data;
        } else if (data.type === 'endpoint:drag') {
          _this19._dragType = 'endpoint:drag';
          _this19._dragEndpoint = data.data;
        } else if (data.type === 'node:move') {
          _this19._moveNode(data.node, data.x, data.y, data.isNotEventEmit);
        } else if (data.type === 'group:move') {
          _this19._moveGroup(data.group, data.x, data.y, data.isNotEventEmit);
        } else if (data.type === 'link:click') {
          _this19._dragType = 'link:click';
        } else if (data.type === 'multiple:select') {
          var result = _this19._selectMultiplyItem(data.range, data.toDirection); // 把框选的加到union的数组


          _$c.assign(_this19._unionData['__system'], _this19.selectItem);

          _this19.emit('system.multiple.select', {
            data: result
          });
          ...
          _this19._dragType = 'group:resize';
          _this19._dragGroup = data.group;
        } else if (data.type === 'node:delete') {
          _this19.removeNode(data.data.id);
        } else if (data.type === 'edge:delete') {
          _this19.removeEdge(data.data);
        } else if (data.type === 'group:delete') {
          _this19.removeGroup(data.data.id);
        } else if (data.type === 'group:addNodes') {
          _$c.get(data, 'nodes', []).forEach(function (item) {
            var _hasNode = _$c.find(_this19.nodes, function (_node) {
              return item.id === _node.id;
            });

            if (!_hasNode) {
              _this19.addNode(item);
            } else {

              var rmItem = _this19.removeNode(item.id, true, true);
          ...
          if (!data.isNotEventEmit) {
            ...
            _this19.emit('system.group.addMembers', {
              nodes: data.nodes,
              group: data.group
            });
          }
          ...
          _$c.get(data, 'nodes', []).forEach(function (item) {
            _this19.removeNode(item.id, true, true);
            ...
            _this19.addNode(item, true);
          });
        } else if (data.type === 'edge:updateLabel') {
          var labelDom = data.data.labelDom;
          $$9(_this19.wrapper).append(labelDom);
        } else if (data.type === 'edge:setZIndex') {
          _this19.setEdgeZIndex([data.edge], data.index);
        }
      }); // 绑定拖动事件
    ...
    key: "_findGroupByCoordinates",
    value: function _findGroupByCoordinates(node, lx, ty, rx, by) {
      for (var i = 0; i < this.groups.length; i++) {
        var _group = this.groups[i];
        var _groupLeft = _group.left;
    ...
    key: "_hoverGroup",
    value: function _hoverGroup(node) {
      var _this20 = this;

      this._hoverGroupQueue.push(node);

      if (!this._hoverGroupTimer) {
        this._hoverGroupTimer = setInterval(function () {
          if (_this20._hoverGroupQueue.length === 0) {
            clearInterval(_this20._hoverGroupTimer);
            _this20._hoverGroupTimer = undefined;
            return;
          }

          var targetNode = _this20._hoverGroupQueue.pop();

          var targetGroup = _this20._findGroupByCoordinates(targetNode, targetNode.left, targetNode.top);
          ...
          if (targetGroup) {
            if (!_this20._hoverGroupObj || targetGroup.id !== _this20._hoverGroupObj.id) {
              _this20._hoverGroupObj && $$9(_this20._hoverGroupObj.dom).removeClass('butterfly-group-hover');
              $$9(targetGroup.dom).addClass('butterfly-group-hover');
              _this20._hoverGroupObj = targetGroup;
            }
          } else {
            _this20._hoverGroupObj && $$9(_this20._hoverGroupObj.dom).removeClass('butterfly-group-hover');
            _this20._hoverGroupObj = undefined;
          }

          _this20._hoverGroupQueue = [];
        }, 200);
      }
    }
  }, {
    key: "_clearHoverGroup",
    value: function _clearHoverGroup(group) {
      this._hoverGroupObj && $$9(this._hoverGroupObj.dom).removeClass('butterfly-group-hover');
    ...
    value: function _attachMouseDownEvent() {
      ...
      var _activeLinkableEndpoint = function _activeLinkableEndpoint(target) {
        ...
        var _allPoints = _this21._getAllEndpoints();
      ...
      var _focusLinkableEndpoint = function _focusLinkableEndpoint(cx, cy) {
        if (!_isFocusing) {
          ...
          _isFocusing = true;
          setTimeout(function () {
            var _points = _this21._getAllEndpoints();

            var x = _this21._coordinateService._terminal2canvas('x', cx);

            var y = _this21._coordinateService._terminal2canvas('y', cy);

            var _focusPoint = null;

            _points.forEach(function (_point) {
              var _maxX = _point._posLeft + _point._width + (_$c.get(_point, 'expandArea.right') || _this21.theme.endpoint.expandArea.right);

              var _maxY = _point._posTop + _point._height + (_$c.get(_point, 'expandArea.bottom') || _this21.theme.endpoint.expandArea.bottom);

              var _minX = _point._posLeft - (_$c.get(_point, 'expandArea.left') || _this21.theme.endpoint.expandArea.left);

              var _minY = _point._posTop - (_$c.get(_point, 'expandArea.top') || _this21.theme.endpoint.expandArea.top);
      ...
      var _clearDraging = function _clearDraging() {
        _this21._dragType = null;
        _this21._dragNode = null;
        _this21._dragEndpoint = null;
        _this21._dragGroup = null;
        _this21._dragEdges = [];
        nodeOriginPos = {
          x: 0,
          y: 0
        ...
        _this21._autoMoveDir = [];
        _this21._guidelineService.isActive && _this21._guidelineService.clearCanvas();
      };
      ...
      var mouseDownEvent = function mouseDownEvent(event) {
        ...
        if (!_this21._dragType && _this21.moveable) {
          _this21._dragType = 'canvas:drag';
        } // 假如点击在空白地方且在框选模式下


        if ((event.target === _this21.svg[0] || event.target === _this21.root) && _this21.isSelectMode) {
          _this21.canvasWrapper.active();

          _this21.canvasWrapper.dom.dispatchEvent(new MouseEvent('mousedown', {
        ...
        if (!_this21._isInitEdgeZIndex) {
          $$9(_this21.svg).css('z-index', _this21._dragEdgeZindex);

          _this21.nodes.forEach(function (item) {
            $$9(item.dom).css('z-index', _this21._dragNodeZIndex * 2 - 1);

            _$c.get(item, 'endpoints').forEach(function (point) {
              $$9(point.dom).css('z-index', _this21._dragNodeZIndex * 2);
            });
          });

          _this21.edges.forEach(function (item) {
            if (item.labelDom) {
              $$9(item.labelDom).css('z-index', _this21._dragEdgeZindex + 1);
            }
          });

          _this21._isInitEdgeZIndex = true;
        } // 拖动的时候提高z-index


        if (_this21._dragNode && _this21._dragNode.__type == 'node') {
          $$9(_this21._dragNode.dom).css('z-index', ++_this21._dragNodeZIndex * 2 - 1);

          _$c.get(_this21._dragNode, 'endpoints').forEach(function (point) {
            $$9(point.dom).css('z-index', _this21._dragNodeZIndex * 2);
          });
        }

        if (_this21._dragNode && _this21._dragNode.__type == 'group') {
          $$9(_this21._dragNode.dom).css('z-index', ++_this21._dragGroupZIndex * 2 - 1);

          _$c.get(_this21._dragNode, 'endpoints').forEach(function (point) {
            $$9(point.dom).css('z-index', _this21._dragGroupZIndex * 2);
          });
        }

        _this21.emit('system.drag.start', {
          dragType: _this21._dragType,
          dragNode: _this21._dragNode,
          dragEndpoint: _this21._dragEndpoint,
          dragEdges: _this21._dragEdges,
          dragGroup: _this21._dragGroup,
          position: {
            clientX: event.clientX,
            clientY: event.clientY,
            canvasX: _this21._coordinateService._terminal2canvas('x', event.clientX),
            canvasY: _this21._coordinateService._terminal2canvas('y', event.clientY)
          }
        });
      ...
      var mouseMoveEvent = function mouseMoveEvent(event) {
        ...
        if (_this21._dragType) {
          var canvasX = _this21._coordinateService._terminal2canvas('x', event.clientX);

          var canvasY = _this21._coordinateService._terminal2canvas('y', event.clientY);

          var offsetX = event.clientX - canvasOriginPos.x;
          var offsetY = event.clientY - canvasOriginPos.y;

          if (_this21._dragType === 'canvas:drag') {
            _this21.move([offsetX + _this21._moveData[0], offsetY + _this21._moveData[1]]);

            canvasOriginPos = {
              x: event.clientX,
              y: event.clientY
            };
          } else if (_this21._dragType === 'node:drag') {
            ...
            if (_this21._dragNode) {
              var moveNodes = [_this21._dragNode];

              var unionKeys = _this21._findUnion('nodes', _this21._dragNode);

              if (unionKeys && unionKeys.length > 0) {
                unionKeys.forEach(function (key) {
                  moveNodes = moveNodes.concat(_this21._unionData[key].nodes);
                });
                moveNodes = _$c.uniqBy(moveNodes, 'id');
              } else {
                _this21._rmSystemUnion();
              }

              $$9(_this21.svg).css('visibility', 'hidden');
              $$9(_this21.wrapper).css('visibility', 'hidden');
              moveNodes.forEach(function (node) {
                _this21._moveNode(node, node.left + (canvasX - nodeOriginPos.x), node.top + (canvasY - nodeOriginPos.y));

                if (_this21._guidelineService.isActive) {
                  _this21._guidelineService.draw(node, 'node');
                }
              });
              $$9(_this21.svg).css('visibility', 'visible');
              $$9(_this21.wrapper).css('visibility', 'visible');
              nodeOriginPos = {
                x: canvasX,
                y: canvasY
              };

              _this21._hoverGroup(_this21._dragNode);

              _this21.emit('system.node.move', {
                nodes: moveNodes
              });
              ...
              _this21._autoMoveCanvas(event.clientX, event.clientY, {
                type: 'node:drag',
                nodes: moveNodes
              }, function (gap) {
          ...
          } else if (_this21._dragType === 'group:drag') {
            if (nodeOriginPos.x === 0 && nodeOriginPos.y === 0) {
              nodeOriginPos = {
                x: canvasX,
                y: canvasY
            ...
            if (_this21._dragNode) {
              $$9(_this21.svg).css('visibility', 'hidden');
              $$9(_this21.wrapper).css('visibility', 'hidden');
              var group = _this21._dragNode;

              _this21._moveGroup(group, group.left + (canvasX - nodeOriginPos.x), group.top + (canvasY - nodeOriginPos.y));

              if (_this21._guidelineService.isActive) {
                _this21._guidelineService.draw(group, 'group');
              }
              ...
              _this21.emit('system.group.move', {
                group: group
              });
          ...
          } else if (_this21._dragType === 'endpoint:drag') {
            var endX = _this21._coordinateService._terminal2canvas('x', event.clientX);

            var endY = _this21._coordinateService._terminal2canvas('y', event.clientY); // 明确标记source或者是没有type且没有线连上


            var _isSourceEndpoint = _this21._dragEndpoint.type === 'source' || _this21._dragEndpoint.type === 'onlyConnect' || !_this21._dragEndpoint.type && (!_this21._dragEndpoint._tmpType || _this21._dragEndpoint._tmpType === 'source');

            var _isTargetEndpoint = (_this21._dragEndpoint.type === 'target' || !_this21._dragEndpoint.type && _this21._dragEndpoint._tmpType === 'target') && _this21._dragEndpoint.type !== 'onlyConnect';

            if (_isSourceEndpoint && _this21.linkable) {
              var _unionKeys = _this21._findUnion('endpoints', _this21._dragEndpoint);

              var edges = [];

              if (!_this21._dragEdges || _this21._dragEdges.length === 0) {
                var EdgeClass = _this21.theme.edge.Class;
                var endpoints = [];

                if (_unionKeys && _unionKeys.length > 0) {
                  _unionKeys.forEach(function (key) {
                    endpoints = endpoints.concat(_this21._unionData[key].endpoints);
                  });
                  ...
                  endpoints = [_this21._dragEndpoint];
                }

                endpoints.forEach(function (point) {
                  var _sourceNode = point.nodeType === 'node' ? _this21.getNode(point.nodeId) : _this21.getGroup(point.nodeId);

                  var _sourceEndpoint = point;
                  var pointObj = {
                    type: 'endpoint',
                    shapeType: _this21.theme.edge.type,
                    orientationLimit: _this21.theme.endpoint.position,
                    _sourceType: point.nodeType,
                    sourceNode: _sourceNode,
                    sourceEndpoint: _sourceEndpoint,
                    arrow: _this21.theme.edge.arrow,
                    arrowPosition: _this21.theme.edge.arrowPosition,
                    arrowOffset: _this21.theme.edge.arrowOffset,
                    label: _this21.theme.edge.label,
                    isExpandWidth: _this21.theme.edge.isExpandWidth
                  };
                  pointObj['options'] = _$c.assign({}, pointObj, {
                    sourceNode: _sourceNode.id,
                    sourceEndpoint: _sourceEndpoint.id
                  }); // 检查endpoint限制连接数目

                  var _linkNums = _this21.edges.filter(function (_edge) {
                  ...
                  var _newEdge = new EdgeClass(_$c.assign(pointObj, {
                    _global: _this21.global,
                    _on: _this21.on.bind(_this21),
                    _emit: _this21.emit.bind(_this21)
                  }));

                  _newEdge._init();

                  $$9(_this21.svg).append(_newEdge.dom);

                  if (_newEdge.labelDom) {
                    $$9(_this21.wrapper).append(_newEdge.labelDom);
                  }

                  if (_newEdge.arrowDom) {
                    $$9(_this21.svg).append(_newEdge.arrowDom);
                  }

                  edges.push(_newEdge);
                });
                _this21._dragEdges = edges;
              } else {
                edges = _this21._dragEdges;
              }
              ...
              if (_this21.theme.endpoint.linkableHighlight) {
                _activeLinkableEndpoint(_this21._dragEndpoint);
            ...
            } else if (_isTargetEndpoint && _this21.disLinkable) {
              // 从后面搜索线
              var targetEdge = null;

              for (var i = _this21.edges.length - 1; i >= 0; i--) {
                if (_this21._dragEndpoint.id === _$c.get(_this21.edges, [i, 'targetEndpoint', 'id']) && _this21._dragEndpoint.nodeId === _$c.get(_this21.edges, [i, 'targetNode', 'id'])) {
                  targetEdge = _this21.edges[i];
                  break;
                }
              }

              if (targetEdge && _this21._dragEdges.length === 0) {
                targetEdge._isDeletingEdge = true;
                _this21._dragEdges = [targetEdge];
              }

              if (_this21._dragEdges.length !== 0) {
                var edge = _this21._dragEdges[0];
                var beginX = edge.sourceEndpoint._posLeft + edge.sourceEndpoint._width / 2;
                var beginY = edge.sourceEndpoint._posTop + edge.sourceEndpoint._height / 2;
                var _soucePoint = {
                  pos: [beginX, beginY],
          ...
          } else if (_this21._dragType === 'group:resize') {
            var _canvasX = _this21._coordinateService._terminal2canvas('x', event.clientX);

            var _canvasY = _this21._coordinateService._terminal2canvas('y', event.clientY);

            var _newWidth = _canvasX - _this21._dragGroup.left;

            var _newHeight = _canvasY - _this21._dragGroup.top;

            _this21._dragGroup.setSize(_newWidth, _newHeight);
          }
        }
      };
      ...
      var mouseEndEvent = function mouseEndEvent(event) {
        ...
        if (_this21._dragType === 'endpoint:drag' && _this21._dragEdges && _this21._dragEdges.length !== 0) {
          // 释放对应画布上的x,y
          var x = _this21._coordinateService._terminal2canvas('x', event.clientX);

          var y = _this21._coordinateService._terminal2canvas('y', event.clientY);

          var _targetEndpoint = null;

          var _nodes = _$c.concat(_this21.nodes, _this21.groups);
          ...
          if (!isDestoryEdges) {
            isDestoryEdges = _$c.some(_this21._dragEdges, function (edge) {
              return !ScopeCompare(edge.sourceEndpoint.scope, _targetEndpoint.scope, _$c.get(_this21, 'global.isScopeStrict'));
            });
          } // 检查endpoint限制连接数目


          if (_targetEndpoint && _targetEndpoint.limitNum !== undefined) {
            var _linkNum = _this21.edges.filter(function (_edge) {
              return _edge.targetEndpoint.id === _targetEndpoint.id;
            }).length + _this21._dragEdges.length;
          ...
          if (isDestoryEdges) {
            _this21._dragEdges.forEach(function (edge) {
              if (edge._isDeletingEdge) {
                _this21.removeEdge(edge);
              } else {
            ...
            _this21._dragEdges.forEach(function (_rmEdge) {
              if (_$c.get(_rmEdge, 'sourceEndpoint._tmpType') === 'source') {
                var isExistEdge = _$c.some(_this21.edges, function (edge) {
                  return _rmEdge.sourceNode.id === edge.sourceNode.id && _rmEdge.sourceEndpoint.id === edge.sourceEndpoint.id;
                });
              ...
              if (_$c.get(_rmEdge, 'targetEndpoint._tmpType') === 'target') {
                var _isExistEdge2 = _$c.some(_this21.edges, function (edge) {
                  return _rmEdge.targetNode.id === edge.targetNode.id && _rmEdge.targetEndpoint.id === edge.targetEndpoint.id;
                });
            ...
            var _emitEdges = _this21._dragEdges.filter(function (edge) {
              // 线条去重
              if (!_this21.theme.edge.isRepeat) {
                var _isRepeat = _$c.some(_this21.edges, function (_edge) {
                  var _result = false;
              ...
              var _currentTargetNode = _targetEndpoint.nodeType === 'node' ? _this21.getNode(_targetEndpoint.nodeId) : _this21.getGroup(_targetEndpoint.nodeId);
            ...
            if (_delEdges.length !== 0 && _emitEdges.length !== 0) {
              ...
              _this21.emit('system.link.reconnect', {
                delLinks: _delEdges,
                addLinks: _emitEdges,
                info: _reconnectInfo
              });
              ...
              if (_delEdges.length !== 0) {
                _delEdges.forEach(function (_edge) {
                  ...
                  _this21.emit('system.link.delete', {
                    link: _edge
                  });
              ...
              if (_emitEdges.length !== 0) {
                _this21.pushActionQueue({
                  type: 'system:addEdges',
                  data: _this21._dragEdges
                });

                _this21.emit('system.link.connect', {
                  links: _this21._dragEdges
                });
        ...
        if (_this21._dragType === 'node:drag' && _this21._dragNode) {
          var _handleDragNode = function _handleDragNode(dragNode) {
            ...
            if (dragNode.group) {
              var _group = _this21.getGroup(dragNode.group);
            ...
            if (!targetGroup) {
              targetGroup = _this21._findGroupByCoordinates(dragNode, _nodeLeft, _nodeTop, _nodeRight, _nodeBottom);
            }
            ...
            if (sourceGroup) {
              ...
              if (sourceGroup !== targetGroup) {
                var rmItem = _this21.removeNode(dragNode.id, true, true);
                ...
                var step = _this21.actionQueue[_this21.actionQueueIndex];
                ...
                _this21.emit('system.group.removeMembers', {
                  group: sourceGroup,
                  nodes: [rmNode]
                });

                if (targetGroup) {
                  if (ScopeCompare(dragNode.scope, targetGroup.scope, _$c.get(_this21, 'global.isScopeStrict'))) {
                    nodeData.top -= targetGroup.top;
                    nodeData.left -= targetGroup.left;
                    nodeData.group = targetGroup.id;
                    nodeData._isDeleteGroup = false;

                    _this21.popActionQueue();
                    ...
                    _this21.emit('system.group.addMembers', {
                      nodes: [rmNode],
                      group: targetGroup
                    });

                    _this21._clearHoverGroup(targetGroup);
                  } else {
              ...
              if (targetGroup) {
                if (ScopeCompare(dragNode.scope, targetGroup.scope, _$c.get(_this21, 'global.isScopeStrict'))) {
                  var _rmItem = _this21.removeNode(dragNode.id, true, true);
                  ...
                  _this21.addNode(_rmNode, true);

                  var _step3 = _this21.actionQueue[_this21.actionQueueIndex];
        ...
        if (_this21._dragType === 'group:resize' && _this21._dragGroup) {
          _this21.emit('events', {
            type: 'system.group.resize',
            group: _this21._dragGroup
          });

          _this21.emit('system.group.resize', {
            group: _this21._dragGroup
          });
        } // 点击空白处触发canvas click，并且框选模式下不触发


        if ((_this21._dragType === 'canvas:drag' || !_this21._dragType) && !_this21.isSelectMode) {
          _this21.emit('system.canvas.click');
        ...
        if (_this21._dragType === 'node:drag' || _this21._dragType === 'group:drag') {
          _this21.pushActionQueue({
            type: '_system:dragNodeEnd'
          });
        }

        _this21.emit('system.drag.end', {
          dragType: _this21._dragType,
          dragNode: _this21._dragNode,
          dragEndpoint: _this21._dragEndpoint,
          dragEdges: _this21._dragEdges,
      ...
      var mouseLeaveEvent = function mouseLeaveEvent(event) {
        if (_this21._dragEdges && _this21._dragEdges.length > 0) {
          _this21._dragEdges.forEach(function (edge) {
            if (edge._isDeletingEdge) {
              _this21.removeEdge(edge);
    ...
    value: function _autoMoveCanvas(x, y, data, cb) {
      ...
      if (!this._autoMoveTimer) {
        ...
        this._autoMoveTimer = setInterval(function () {
          if (_this22._autoMoveDir.includes('left')) {
            _this22.move([_this22._moveData[0] + MOVE_GAP, _this22._moveData[1]]);

            _moveTarget([-MOVE_GAP, 0]);
          }

          if (_this22._autoMoveDir.includes('right')) {
            _this22.move([_this22._moveData[0] - MOVE_GAP, _this22._moveData[1]]);

            _moveTarget([+MOVE_GAP, 0]);
          }

          if (_this22._autoMoveDir.includes('top')) {
            _this22.move([_this22._moveData[0], _this22._moveData[1] + MOVE_GAP]);

            _moveTarget([0, -MOVE_GAP]);
          }

          if (_this22._autoMoveDir.includes('bottom')) {
            _this22.move([_this22._moveData[0], _this22._moveData[1] - MOVE_GAP]);
    ...
    value: function _moveNode(node, x, y, isNotEventEmit) {
      var _isInGroup = !!node.group;

      if (_isInGroup) {
        var groupObj = this.getGroup(node.group);
        var groupType = groupObj.type || this.theme.group.type;

        if (groupType === 'inner') {
          var _groupW = $$9(groupObj.dom).width();

          var _groupH = $$9(groupObj.dom).height();

          var _nodeW = $$9(node.dom).width();

          var _nodeH = $$9(node.dom).height();
      ...
      node.endpoints && node.endpoints.forEach(function (item) {
        item.updatePos();
      });
      this.edges.forEach(function (edge) {
    ...
    value: function _autoLayout(data) {
      ...
      if (_$c.isFunction(this.layout)) {
        ...
        if (_$c.get(this.layout, 'type') === 'forceLayout') {
        ...
        } else if (_$c.get(this.layout, 'type') === 'radial') {
          var _opts4 = $$9.extend({
            // 布局画布总宽度
            width: _$c.get(this.layout, 'options.width') || 500,
            // 布局画布总长度
            height: _$c.get(this.layout, 'options.height') || 500,

            /** 停止迭代的最大迭代数 */
            maxIteration: _$c.get(this.layout, 'options.maxIteration') || 1000,

            /** 布局中心 */
            center: _$c.get(this.layout, 'options.center') || [width / 2, height / 2],

            /** 中心点，默认为数据中第一个点 */
            focusNode: _$c.get(this.layout, 'options.focusNode') || null,

            /** 每一圈半径 */
            unitRadius: _$c.get(this.layout, 'options.unitRadius') || null,

            /** 默认边长度 */
            linkDistance: _$c.get(this.layout, 'options.linkDistance') || 50,

            /** 是否防止重叠 */
            preventOverlap: _$c.get(this.layout, 'options.preventOverlap') || false,

            /** 节点直径 */
            nodeSize: _$c.get(this.layout, 'options.nodeSize') || undefined,

            /** 节点间距，防止节点重叠时节点之间的最小距离（两节点边缘最短距离） */
            nodeSpacing: _$c.get(this.layout, 'options.nodeSpacing') || undefined,

            /** 是否必须是严格的 radial 布局，即每一层的节点严格布局在一个环上。preventOverlap 为 true 时生效 */
            strictRadial: _$c.get(this.layout, 'options.strictRadial') || true,

            /** 防止重叠步骤的最大迭代次数 */
            maxPreventOverlapIteration: _$c.get(this.layout, 'options.maxPreventOverlapIteration') || 200,
            sortBy: _$c.get(this.layout, 'options.sortBy') || undefined,
            sortStrength: _$c.get(this.layout, 'options.sortStrength') || 10,
            link: {
              // 以node的什么字段为寻找id，跟d3原理一样
              id: 'id',
              // 线条的距离
          ...
          if (_$c.get(this.layout, 'type') === 'radial') {
            Layout$1.radial({
              opts: _opts4,
              data: {
                groups: data.groups,
    ...
    value: function _selectMultiplyItem(range, toDirection) {
      ...
      var _isSelected = function _isSelected(option) {
        ...
        if (_this23.selecMode === 'include' || _this23.selecMode === 'senior' && toDirection === 'right') {
          return startX < _itemLeft && endX > _itemRight && startY < _itemTop && endY > _itemBottom;
        }

        if (_this23.selecMode === 'touch' || _this23.selecMode === 'senior' && toDirection === 'left') {
          var result = true;
      ...
      if (includeNode) {
        this.nodes.forEach(function (item) {
          ...
          if (item.group) {
            var _group = _this23.getGroup(item.group);
          ...
          if (isSelected) {
            _this23.selectItem.nodes.push(item);
          }
        });
      } // 框选锚点
      ...
      if (includeEndpoint) {
        this.nodes.forEach(function (node) {
          node.endpoints.forEach(function (item) {
            ...
            if (isSelected) {
              _this23.selectItem.endpoints.push(item);
            }
          });
        });
      } // 框选线条
      ...
      if (includeEdge) {
        this.edges.forEach(function (item) {
          if (item.type === 'endpoint') {
            ...
            if (isSelected) {
              _this23.selectItem.edges.push(item);
            }
      ...
      if (step.type === 'system:addNodes') {
        ...
        step.data.forEach(function (item) {
          if (item.nodes.length > 0) {
            _this24.removeNodes(item.nodes);
          }

          _this24.removeGroup(item.group.id, true);
        });
        ...
        _$c.get(step, 'data.info', []).forEach(function (info) {
          var targetNode = _this24.getNode(info.preTargetNodeId);
    ...
    value: function redo() {
      ...
      if (step.type === 'system:addNodes') {
        ...
        step.data.forEach(function (item) {
          _this25.addGroup(item.group, item.nodes || [], undefined, true);
        });
      } else if (step.type === 'system:removeGroup') {
        ...
        _$c.get(step, 'data.info', []).forEach(function (info) {
          var targetNode = _this25.getNode(info.currentTargetNodeId);
    ...
    value: function pushActionQueue(option) {
      ...
      if (option.type === 'system:moveNodes' || option.type === 'system:moveGroups') {
        ...
        if (currentStep.type === option.type && currentStep.data[_types][option.data[_type].id]) {
          ...
          if (unionKeys && unionKeys.length > 0) {
            unionKeys.forEach(function (key) {
              moveItems = moveItems.concat(_this26._unionData[key][_types]);
            });
    ...
    value: function setEdgeZIndex() {
      ...
      edges.forEach(function (edge) {
        ...
        var index = _$c.findIndex(_this27.edges, function (item) {
          return item === edge;
        });

        if (index !== -1) {
          var delEdge = _this27.edges.splice(index, 1);

          $$9(delEdge.dom).detach();
          delEdge.eventHandlerDom && $$9(delEdge.eventHandlerDom).detach();
          delEdge.arrowDom && $$9(delEdge.arrowDom).detach();
        } else {
          return;
        }
      });

      var addIndex = this._findEdgeIndex(edges[0]);
      ...
      var beforeEdge = this.edges[addIndex];
      var afterEdge = this.edges[addIndex + 1];

      if (beforeEdge) {
        var targetDom = beforeEdge.dom;
        beforeEdge.eventHandlerDom && (targetDom = beforeEdge.eventHandlerDom);
        beforeEdge.arrowDom && (targetDom = beforeEdge.arrowDom);
        $$9(targetDom).after(addEdgesDom);
      } else if (afterEdge) {
        $$9(afterEdge.dom).before(addEdgesDom);
      } else {
        $$9(this.svg).append(addEdgesDom);
      }

      (_this$edges = this.edges).splice.apply(_this$edges, [addIndex + 1, 0].concat(_toConsumableArray(edges)));
    }
  }, {
    key: "_findEdgeIndex",
    value: function _findEdgeIndex(edge) {
      var _this28 = this;

      var index = 0;
      var currentZIndex = edge._zIndex || 0;
...
var DEFUALT = {
  TEMPLATE: '<div class="butterfly-tooltip-container"><div class="butterfly-tooltip-arrow"></div><div class="butterfly-tooltip-inner"></div></div>',
  $viewAppend: 'body',
  $viewCon: {
    tips: '.butterfly-tips',
    menu: '.butterfly-menu',
...
var show = function show(opts, type, tipsDom, targetDom, callback) {
  $$a(DEFUALT.$viewCon[type]).remove();
  var tipsContainer = $$a(DEFUALT.TEMPLATE);
  tipsContainer.find(DEFUALT.$inner).append(tipsDom);
  $$a(tipsContainer).appendTo(DEFUALT.$viewAppend);
  var placement = opts.placement || 'top';
  $$a(tipsContainer).addClass(DEFUALT.$viewCon[type].replace('.', '')).addClass(placement).addClass('in'); // todo in的动画
  ...
  var pos = {
    top: $$a(targetDom).offset().top,
    left: $$a(targetDom).offset().left,
    width: $$a(targetDom).outerWidth(),
    height: $$a(targetDom).outerHeight(),
    actualWidth: $$a(tipsContainer).outerWidth(),
    actualHeight: $$a(tipsContainer).outerHeight()
  };
  var posInit = {};
  ...
  $$a(tipsContainer).attr('style', position);
  callback && callback(tipsContainer[0]);
  return tipsContainer[0];
};

var hide = function hide(tipsDom, callback) {
  $$a(tipsDom).removeClass('in').remove();
  callback && callback(tipsDom);
};

var createTip = function createTip(opts, callback) {
  var currentTips = null;
  var data = opts.data,
      targetDom = opts.targetDom,
      genTipDom = opts.genTipDom;
  targetDom.addEventListener('mouseover', function (e) {
    // e.preventDefault();
    // e.stopPropagation();
    var tipstDom = genTipDom(data);
...
var _hideMenu = function _hideMenu(e) {
  if (e.target === currentMenu || $$a(currentMenu).find(e.target).length > 0) {
    return;
  }
...
var createMenu = function createMenu(opts, callback) {
  var data = opts.data,
      targetDom = opts.targetDom,
      genTipDom = opts.genTipDom;

  var _createMenu = function _createMenu() {
    var tipstDom = genTipDom(data);
...
var closeMenu = function closeMenu(callback) {
